...
// 右パネルの編集は「今すぐ録音(ad-hoc)」のときだけ許可する（本機能の対象は ad-hoc）。
// 予定(?id=)を開いた録音では、アジェンダ/参加者/事前資料は会議作成(S-02)で確定済み＝読み取り専用に戻す。
// linked で編集を許すと保存経路で黙って捨てられ（complete_meeting は agenda/participants を更新しない）、
// 添付の即時DB反映が「破棄」と食い違うため（コードレビュー指摘）。
const editable = computed<boolean>(() => !linkedMeetingId.value);
...
const pickAndAttach = async (): Promise<void> => {
  ...
  try {
    ...
    for (const path of paths) {
      ...
      // add_attachment は抽出完了までブロックするので、先に pending 行を楽観表示し「解析中…」を見せる。
      const id = crypto.randomUUID();
      ...
      try {
        const a = await addAttachment(id, mid, path, name, type, createdAt);
        const i = attachments.value.findIndex((x) => x.id === id);
        if (i >= 0) attachments.value[i] = a;
        else attachments.value.push(a);
      } catch (e) {
        attachments.value = attachments.value.filter((x) => x.id !== id); // pending 行を撤回
        $q.notify({ message: `資料の取り込みに失敗しました: ${e}`, color: "negative", icon: "error" });
      }
    }
  } catch (e) {
    $q.notify({ message: `資料の準備に失敗しました: ${e}`, color: "negative", icon: "error" });
  } finally {
...
const endMeeting = async (): Promise<void> => {
  ...
  if (linkedMeetingId.value) {
    ...
    minutesSession.isTempMeeting = false; // ← 予定は仮会議ではない。前セッションの残存フラグを必ず落とす（重要・データ破壊防止）
    ...
    try {
      ...
      // 書き戻し失敗時は作成時の値で保存される。黙らせず通知して気づけるようにする（コードレビュー指摘）。
      $q.notify({
        message: `アジェンダ/参加者の保存に失敗しました（録音内容は保存できます）: ${e}`,
        color: "warning",
        icon: "warning",
        timeout: 3000,
      });
  ...
  <q-layout view="hHh LpR fFf">
    ...
    <q-drawer side="right" v-model="drawer" show-if-above bordered :width="280">
      <q-scroll-area class="fit">
        <q-list padding>
          <!-- アジェンダ: ad-hoc は録音中もその場で編集（DD-016-3）。予定(linked)は読み取り専用。 -->
          <q-item-label header class="row items-center">
            アジェンダ <q-space />
            <q-icon v-if="editable" name="edit" size="16px" color="grey-6">
              <q-tooltip>録音中も編集できます</q-tooltip>
            </q-icon>
          </q-item-label>
          <q-item>
            <q-item-section>
          ...
          <q-separator spaced />
          <!-- 事前資料（DD-012-10/案C・DD-016-3）: ad-hoc は録音中に追加でき清書に反映。予定(linked)は読み取り専用。 -->
          <q-item-label header class="row items-center">
            事前資料 <q-space />
            <q-icon v-if="editable" name="edit" size="16px" color="grey-6">
              <q-tooltip>録音中も追加できます</q-tooltip>
            </q-icon>
          </q-item-label>
          ...
          <q-item v-for="a in attachments" :key="a.id">
            ...
            <q-item-section side class="row items-center no-wrap">
              ...
              <q-btn v-if="editable" flat round dense size="sm" icon="close" color="grey-6" @click="removeAttachmentRow(a)">
                <q-tooltip>削除</q-tooltip>
              </q-btn>
            </q-item-section>
          </q-item>
