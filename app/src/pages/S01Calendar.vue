...
const load = async (): Promise<void> => {
  setActive({ screen: `カレンダー（${monthLabel.value}）` }); // 会議は無し（チップ非表示）・OSタイトルに表示中の月を出す
  loading.value = true;
  errorMsg.value = "";
  try {
    const y = year.value;
...
// ===== 操作メニュー =====
// completed→議事録詳細(/s03 に id)、進行中/生成中→会議画面(/s05 に id)、その他→会議作成(/s02)
// /s05 に id を渡すことで、録音→保存がその予定に紐づく（保存先が今日にずれる不具合の修正）。
