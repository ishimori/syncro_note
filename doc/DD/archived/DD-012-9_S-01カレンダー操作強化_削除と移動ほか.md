...
### Phase 5: 実機 E2E 確認 ✅
- [x] 🔬 実ウィンドウで一周（新規→移動→編集→削除→元に戻す→コピー）＝**ユーザー実機で全項目OK**
- [x] 🐞 実機で発覚→修正: ①**ドラッグ不可**＝Tauri 既定の OS ドロップ横取り（`dragDropEnabled`）が webview の HTML5 DnD を無効化 → `tauri.conf.json` で `dragDropEnabled:false`。チップは clickable リップルが DnD 開始を飲むため**外側 div で drag を受ける**構造に。②**保存後に無通知** → S-02 に成功トースト（タイトル＋日付＋時刻を caption 表示）
- [x] 😈 総合 DA → 下記 Phase 5 DA
...
### 2026-06-08
- **Phase 5 完了＝DD クローズ**: 実ウィンドウで一周（新規作成→ドラッグ移動＋元に戻す→予定編集→完了議事録編集(参加者ロック)→削除＋元に戻す→コピー書き出し）を**ユーザー実機で全項目OK**。実機で2点修正: ①ドラッグが効かない＝Tauri 既定の `dragDropEnabled`(OSドロップ横取り)が HTML5 DnD を無効化 → `tauri.conf.json` に `dragDropEnabled:false`（＋チップを外側 div で drag 受け）。②保存後に無通知 → S-02 に成功トースト（タイトル＋日付＋時刻 caption）。**注意**: `dragDropEnabled:false` は OS からのファイルドロップを無効化するため、DD-012-10（Excel/PDF添付のD&D取り込み）着手時に Tauri drag-drop イベント方式で両立を再設計する。
...
**DA観点:** （webview と OS の DnD 競合／設定変更の副作用）

| # | 発見した問題/改善点 | 重要度 | 再現手順（高/中は必須） | DA観点 | 対応 |
|---|-------------------|--------|----------------------|--------|------|
| 15 | **HTML5 DnD が実機で無効**: Tauri 既定 `dragDropEnabled=true` が webview のドラッグイベントを OS 側で横取り（ブラウザでは動くが実ウィンドウで動かない罠） | 高 | 実ウィンドウでチップをドラッグ | webview/OS 競合 | `tauri.conf.json` の window に `dragDropEnabled:false`。設定変更は HMR 不可＝`tauri dev` 再起動が要る点も周知 |
| 16 | **`dragDropEnabled:false` の副作用**: OS からウィンドウへのファイルドロップが無効になる | 中 | エクスプローラからファイルをドロップ | 機能退行 | 現状ファイルドロップ未使用なので可。**DD-012-10（添付D&D）着手時に Tauri の `tauri://drag-drop` イベント方式で両立を再設計**（本DDログに申し送り） |
